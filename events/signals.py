...
# =========================================================
# 🖼️ Legacy Identity: Image Poster Controls
# =========================================================
...
# Tentative d'initialisation de Firebase (ne plante pas si le fichier n'existe pas)
try:
    ...
    if os.path.exists(cred_path):
        ...
        print("⚠️ Firebase credentials file not found. Push notifications disabled.")
except ImportError:
...
def send_fcm_push(recipient, title, body):
    ...
    """
    # Vérifie que l'utilisateur a un token FCM enregistré
    ...
    try:
        ...
        print(f"✅ FCM sent to {recipient.username}: {response}")
        return response
    except Exception as e:
...
# =========================================================
# 🎯 TASK 3: Real-Time Booking & Cancellation Listeners
# =========================================================
...
def notify_organizer_on_booking(sender, instance, created, **kwargs):
    ...
    if created:
        ...
        if event and event.organizer:
            ...
            # 1. Sauvegarde en base de données (historique)
            ...
            # 2. Envoi du push Firebase en temps réel

...
def verification_pending(request):
    return render(
        request,
        'authentication/verification_pending.html',
    )
...
def verify_email(request, uidb64, token):
    try:
        ...
        return render(
            request,
            'authentication/email_verification_invalid.html',
        )

    if user.email_verified:
        return render(
            request,
            'authentication/email_verification_success.html',
            {'user': user},
        )
...
def login_view(request):
    if request.method == 'POST':
        ...
        if user is None:
            ...
            # Admin accounts are exempt from email verification.
...
def organizer_dashboard(request):
    """Dashboard showing ticket sales performance, revenue, and charts."""
    ...
    total_tickets_remaining = sum(
        event.tickets_remaining
        for event in events
    )

    total_revenue = sum(
        (event.revenue for event in events),
        Decimal('0.00'),
    )
    ...
    context = {
        ...
        'chart_tickets_sold_json': json.dumps(
            chart_tickets_sold
        ),
        'chart_tickets_remaining_json': json.dumps(
            chart_tickets_remaining
        ),
...
def organizer_dashboard_stats_api(request):
    """Return live organizer statistics and chart data."""
    events = Event.objects.filter(
        organizer=request.user
    ).order_by('-date')
...
def organizer_export_excel(request):
    """Download Excel analytics for organizers or admins."""
    if not (
...
def organizer_export_pdf(request):
    """Download styled PDF analytics report."""
    if not (
        request.user.is_organizer
        or request.user.is_admin
    ):
...
def admin_user_list(request):
    ...
    return render(
        request,
        'authentication/admin_user_list.html',
        {'users': users},
    )
...
def organizer_request_approve(request, pk):
    ...
    organizer.save(
        update_fields=['organizer_status']
    )

    messages.success(
        request,
        f'Organizer "{organizer.username}" was approved '
        'and can now publish events.',
    )
...
def organizer_request_deny(request, pk):
    ...
    messages.success(
        request,
        f'Organizer request from "{organizer.username}" '
        'was denied.',
    )
...
def event_request_approve(request, pk):
    ...
    event.save(
        update_fields=['publish_status']
    )

    messages.success(
        request,
        f'Event "{event.title}" was approved '
        'and is now live in the system.',
    )

    return _redirect_after_event_review(
        request,
        event,
    )
...
def event_request_deny(request, pk):
    ...
    messages.success(
        request,
        f'Event "{event.title}" was denied '
...
def my_bookings(request):
    ...
    upcoming_legacy = [
        booking
        for booking in legacy_bookings.filter(
            event__date__gte=now
        ).order_by('event__date')
        if not any(
    ...
    upcoming_bookings = sorted(
        upcoming_tickets + upcoming_legacy,
        key=lambda booking: booking.event.date,
    )
    ...
    return render(
        request,
        'authentication/my_bookings.html',
        {
            'upcoming_bookings': upcoming_bookings,
...
def cancel_booking(request, pk):
    ticket = get_attendee_cancellable_booking(
        request.user,
        pk,
    )
    ...
    if request.method == 'POST':
        ...
        if (
            cancel_quantity < 1
            or cancel_quantity > ticket.quantity
        ):
            messages.error(
                request,
                'Invalid ticket quantity to cancel.',
            )
        ...
        if cancel_quantity >= ticket.quantity:
            try:
                ...
                messages.success(
                    request,
                    'Booking cancelled and cancellation '
                    'email sent successfully.',
                )

            except Exception as exc:
                print(
                    f'CANCELLATION EMAIL ERROR: {exc}'
                )

                messages.warning(
                    request,
                    'Booking was cancelled, but the '
            ...
            messages.success(
                request,
                f'Cancelled {cancel_quantity} ticket(s). '
                f'{ticket.quantity} remaining.',
            )
...
def analytics_dashboard(request):
    ...
    total_tickets_sold = tickets.aggregate(
        total=Coalesce(
            Sum('quantity'),
            0,
        )
    ...
    return render(
        request,
        'authentication/analytics_dashboard.html',
        context,
    )